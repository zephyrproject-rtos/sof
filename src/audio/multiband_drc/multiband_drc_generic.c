...
static void multiband_drc_s16_default_pass(const struct comp_dev *dev,
...
static void multiband_drc_s32_default_pass(const struct comp_dev *dev,
					   const struct audio_stream *source,
					   struct audio_stream *sink,
					   uint32_t frames)
{
				      ...
				      uint32_t frames)
{
	...
	for (i = 0; i < frames; ++i) {
		...
		for (ch = 0; ch < nch; ch++) {
			y = audio_stream_read_frag_s16(sink, idx_sink);
				      ...
				      uint32_t frames)
{
	...
	for (i = 0; i < frames; ++i) {
		...
		for (ch = 0; ch < nch; ch++) {
			y = audio_stream_read_frag_s32(sink, idx_sink);
...
/* { SOURCE_FORMAT , PROCESSING FUNCTION } */
#if CONFIG_FORMAT_S16LE
	{ SOF_IPC_FRAME_S16_LE, multiband_drc_s16_default_pass },
#endif /* CONFIG_FORMAT_S16LE */

#if CONFIG_FORMAT_S24LE
	{ SOF_IPC_FRAME_S24_4LE, multiband_drc_s32_default_pass },
#endif /* CONFIG_FORMAT_S24LE */

#if CONFIG_FORMAT_S32LE
	{ SOF_IPC_FRAME_S32_LE, multiband_drc_s32_default_pass },
#endif /* CONFIG_FORMAT_S32LE */
};
